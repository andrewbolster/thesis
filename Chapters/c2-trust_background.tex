...
\fi
In this chapter we explore the current literature and research on the concepts, theory, and applications concerning Trust and Trust Management, specifically leaning towards the applications of Trust within Autonomous \glspl{manet}.
...
In the third section, current use and applications of Trusted operation of \glspl{manet} is explored, including current \glspl{tmf}.\todo{section references}


\section{Trust Definitions and Perspectives}
\todo[inline]{Clarify Trust vs trust (and check consistency of use)}
...
As such it is important to explore the following areas of Trust definition before approaching the application of Trust towards Autonomous Systems and finally to \glspl{manet}:
...
This formulation of trust allowed a wider discussion of the characteristics of trust relationships, both between individuals and within networks or communities.\todo{possibly expand this}
...
Sun\cite{Sun2008} suggests that there are two overarching forms of trust:
\begin{itemize}
...
\subsection{Characteristics of Trust Relationships}\todo{Needs Citations}
...
Beyond the attributes or characteristics of an individual trust relationship, within any multi party sparsely \todo{First References to Sparcity!} connected network or community, topological context is useful in both establishing trust and in disseminating observations for collaborative assessment.
...
\begin{itemize}
  ...
  \item \emph{Recommendation} -  Where three nodes are fully connected so as to enable the exchange of direct opinions and form composite opinions based on the target and reporter (i.e.\ $A$ has both its own Direct assessment of $C$, as well as it's knowledge of $B$s Direct assessment of $C$)\todo{Possibly worth incorporating the transitive property to this}
\end{itemize}
\todo{Separate Figure for each relationship}
...
\todo{Section on Autonomy in Humanity / Society / Nations}

Autonomy, like trust, is a nebulous, ill defined term across research, defense and commercial circles.\todo{This is unfair}
...
\begin{table}\centering
  ...
  \end{tabularx}
\end{table}\todo{Check this against source, specifically MC definition wrt action vs option}


\todo[inline]{Discuss levels of autonomy}
...
\todo[inline]{Rethink using these questions at all; opens up to awkward questioning that isn't ansered in the thesis}
\begin{enumerate}
...
\subsection{Human Factors related to Operational Trust}\label{sec:human_factors}
...
Trust in the autonomous or semi-autonomous realm is the ability of a system to establish and maintain confidence in itself or another systems' operations.\todo{These Definitions a) don't exist anywhere else and b) conflict, dingus}

There are five topics that are important to address in any \glspl{manet} trust model \cite{Kamvar2003}:\todo{inapprops citation}
%
\begin{itemize}
...
This predictive behaviour attempts to solve four important problems (paraphrased from \cite{Sun2008}):\todo{What predictive behaviour?}
\begin{itemize}
