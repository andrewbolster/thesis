...
Simulations were conducted using a Python based simulation framework, SimPy \cite{Mueller2003SimPy}, with a network stack built upon \gls{auv}NetSim \cite{Miquel2008}, with transmission parameters (\autoref{tab:sysconstraints}) taken from and validated against \cite{Stojanovic2007}, \cite{Stefanov2011} and \cite{Sehgal2010}\todo{it would be worth while going through this verification explicitly as an appendix}
...
Initial node separation was set as per \citet{Guo11} at 100$m$, and each simulation is run 16 times, with each instance modelling a 8 hour mission time.
...
According to \citet{Xu2002}, RTS/CTS handshake functionality cannot operate well as interference protection at node separations beyond 0.56 times the transmission range\cite{Xu2002}.
...
However, when all nodes are randomly walking the saturation point collapses from $0.025pps$ at $300m$ to $0.015pps$ at $400m$.\todo[inline]{Double Check These Saturation Points Before Release}
...
\end{figure*}
%
The use of \gls{fbr} and a \gls{csmaca} \gls{mac} scheme from \gls{auv}NetSim \cite{Miquel2008} in our simulation mitigates a significant number of packet losses through collision avoidance and contention handling, leading to the situation that the only genuinely lost packets occur when a node moves completely out of range of any other node and time out occurs in route discovery rather than transmission (See \autoref{sec:manet_routing}).
...
\end{figure}
\todo{Malicious Factors graph is out of date and shouldn't have physical metrics after rearrange. Also needs vector}
