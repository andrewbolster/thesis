...
%DIF DEL head.tex                 Wed Apr 15 13:31:41 2015
%DIF ADD trustcom15_bolster.tex   Wed Apr 15 13:30:11 2015
%% bare_conf.tex
%% V1.3
%% 2007/01/11
%% by Michael Shell
...
\title{\DIFdelbegin \DIFdel{Applicability of }\DIFdelend Single and Multi-Metric Trust Management Frameworks \DIFaddbegin \DIFadd{for use }\DIFaddend in Underwater Autonomous Networks \\ {\large DRAFT for 8pg submission to \DIFdelbegin \DIFdel{TrustCom 31 Mar}\DIFdelend \DIFaddbegin \DIFadd{TrustCom20 Apr}\DIFaddend }}
...
\begin{abstract}
%\boldmath
In this paper, we demonstrate the need for multi-metric trust assessment in Underwater Autonomous Networks (UAN)\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend a performance comparison against a selection of current single metric Trust Management Framework (TMF) assessment processes. 
\DIFaddbegin \DIFadd{Many UANs use MANET architectures, however the marine environment presents new challenges for TMFs that have been developed for use in conventional (i.e. Terrestrial RF MANETs)
}\DIFaddend We investigate the operation of a selection of traditional MANET TMFs \DIFdelbegin \DIFdel{(Hermes, OTMF) }\DIFdelend in this environment\DIFdelbegin \DIFdel{, highlighting the challenges in this, and demonstrating }\DIFdelend \DIFaddbegin \DIFadd{.
We characterise these challenges and present results that demonstrate }\DIFaddend that a multi-metric approach to Trust greatly enhances the usefulness of Trust in these environments.
...
\DIFaddbegin \todo{Acoustic Marine Environment Bit}
...
In Section \ref{sec:trustandtmfs} we discuss Trust and \DIFdelbegin \DIFdel{Trust Management Frameworks}\DIFdelend \DIFaddbegin \DIFadd{TMFs}\DIFaddend , defining our terminology and reviewing the justifications for the use and development of \DIFdelbegin \DIFdel{Trust Management Frameworks in }\DIFdelend \DIFaddbegin \DIFadd{TMFs for }\DIFaddend marine acoustic networks\DIFaddbegin \todo{Check consistency of use of this term}\DIFaddend .
...
In Section \ref{sec:trustresultsanddiscussion}, we present our findings in trust establishment and malicious behaviour detection, comparing with other current TMFs (\DIFdelbegin \DIFdel{OTMF and Beta}\DIFdelend \DIFaddbegin \DIFadd{Hermes and OTMF}\DIFaddend ) and analyse the use of this multi-parameter approach to detecting malicious and selfish behaviour in autonomous marine networks.

The contributions of this paper are a study on the comparative operation and performance \DIFdelbegin \DIFdel{between terrestrial and underwater MANETs using TMFs }\DIFdelend \DIFaddbegin \DIFadd{of TMFs in marine acoustic networks}\DIFaddend , and a review of metric suitability for TMFs in marine environments, informing future metric selection for experimenters and theorists.
We also show that single metric trust systems are not directly suitable for the marine context in \DIFdelbegin \DIFdel{therms }\DIFdelend \DIFaddbegin \DIFadd{terms }\DIFaddend of the different threat and cost scenario in that environment.

\section{Trust and Trust Management Frameworks}\label{sec:trustandtmfs}

\subsection{Trust in \DIFaddbegin \DIFadd{Conventional `}\DIFaddend MANETs}\label{sec:trustinmanets}
...
\DIFaddbegin \todo{Why select Hermes and OTMF}
...
The attenuation that occurs in an underwater acoustic channel over a distance $d$ for a signal about frequency $f$ in linear power is given as $A_{\text{aco}}(d,f) = A_0d^ka(f)^d$ and in $dB$ form as \DIFdelbegin %DIFDELCMD < \eqref{eq:acoattenuation} 
%DIFDELCMD < %%%
...
\subsection{Trust in Marine Networks}\label{sec:trust_in_marine}\DIFaddbegin \todo{Move to Section \ref{sec:trustandtmfs}}
\DIFaddend 
...
The use of six nodes and the particular layout enables the investigation of the three trust relationships based on minimum path topologies\DIFdelbegin %DIFDELCMD < \todo{Possibly cite Guo paper on team sizes or find an alternative, but would require tangential discussion of team scaling, which there isn't room for}%%%
...
\DIFaddbegin \todo{Possibly cite Guo paper on team sizes or find an alternative, but would require tangential discussion of team scaling, which there isn't room for}
\todo{Include NATO/CMRE LaSpezia justification statement}
\DIFaddend 
...
\DIFaddbegin \todo{Move this to Sec.~\ref{sec:multimetrictrust}}
\DIFaddend 
...
\begin{figure}[h]
  \centering
  \subfloat[Static]{\includegraphics[width=0.5\linewidth]{img/throughput_sep_lines_static} \label{fig:throughput_static}}
  \subfloat[Mobile]{\includegraphics[width=0.5\linewidth]{img/throughput_sep_lines_all_mobile} \label{fig:throughput_all_mobile}}
...
\end{figure}\DIFaddbegin \todo{CULL}
\DIFaddend %
...
\DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \begin{enumerate}
  \item \DIFaddend Malicious Power Control \DIFaddbegin \DIFadd{(MPC) }\DIFaddend behaviour, where $n_1$ increases it's transmission power by 20\% for all nodes \emph{except} communications with $n_0$ in order to make $n_0$ appear ``bad'' to the rest of the team,
  \DIFdelbegin \DIFdel{and a }\DIFdelend \DIFaddbegin \item \DIFaddend Selfish Target Selection \DIFdelbegin \DIFdel{behaviour where nodes preferentially communicate, forward and advertise }\DIFdelend \DIFaddbegin \DIFadd{(STS) behaviour, where $n_1$ preferentially communicates, forwards and advertises }\DIFaddend to nodes that are physically close to \DIFdelbegin \DIFdel{them }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend in effort to reduce \DIFdelbegin \DIFdel{overall power consumptionfor the individual.
}\DIFdelend \DIFaddbegin \DIFadd{$n_1$'s power consumption.
}\end{enumerate}
\DIFaddend 


\section{\DIFdelbegin \DIFdel{Trust}\DIFdelend \DIFaddbegin \DIFadd{Simulation Results and Discussion}\DIFaddend }\label{sec:trustresultsanddiscussion}
...
We also include aggregate assessments; $T_{1,\text{Avg}}$, the \DIFdelbegin \DIFdel{flat average }\DIFdelend \DIFaddbegin \DIFadd{unweighted mean }\DIFaddend of direct trust assessments of $n_1$, $T_{1,\text{Net}}$, that weights assessments according to the network topology from \eqref{eq:networkeffects}, without the \DIFdelbegin \DIFdel{whitenization }\DIFdelend \DIFaddbegin \DIFadd{whitenisation }\DIFaddend factor $f_s$, and $T_{1,\text{MTFM}}$, the final MTFM trust assessment value based on both network topology and \DIFdelbegin \DIFdel{whitenization }\DIFdelend \DIFaddbegin \DIFadd{whitenisation }\DIFaddend from \eqref{eq:whitenization}.\todo{It's possible this paragraph is surplus to requirements and the relevant sections of the boxplot could be removed to simplify things}
...
The aggregate trust values using topology information ($T_{1,\text{Net}},T_{1,\text{MTFM}}$) display a decreased variation than those of the individual subjective observations in both cases.
...
\begin{figure}[h]
  \centering
  \subfloat[Fair Static]{\includegraphics[width=0.45\linewidth]{img/trust_bella_static_fair} \label{fig:trust_static}}
  \subfloat[Fair Mobile]{\includegraphics[width=0.45\linewidth]{img/trust_bella_all_mobile_fair}  \label{fig:trust_all_mobile}}
  \newline
  \subfloat[Selfish Static]{\includegraphics[width=0.45\linewidth]{img/trust_bella_static_selfish} \label{fig:trust_static}}
  \subfloat[Selfish Mobile]{\includegraphics[width=0.45\linewidth]{img/trust_bella_all_mobile_selfish}  \label{fig:trust_all_mobile}}
  \caption{MTFM Trust assessments of $n_1$ ($T_{1,X}$)\protect\footnotemark}\todo{Discuss differential of n0 ratings, also this is impossible to read} 
  \label{fig:trust_mobility}
\end{figure}
\DIFaddbegin \todo{MAYBE CULL}
\DIFaddend %
...
\begin{figure}[h]
  ...
  \subfloat[Selective Power Control Scenario]{\includegraphics[width=.5\linewidth]{img/trust_beta_otmf_selfish} \label{fig:all_mobile_badmouthing_beta}}
...
Fig.~\ref{fig:otmf_beta_comparison} shows a comparison between the unweighted response of MTFM compared to OTMF and \DIFdelbegin \DIFdel{Beta }\DIFdelend \DIFaddbegin \DIFadd{Hermes }\DIFaddend assessment functions on the same data for the fair and selfish behaviours respectively.
...
In this case, particularly since the method of attack was not directly related to PLR, OTMF and \DIFdelbegin \DIFdel{Beta }\DIFdelend \DIFaddbegin \DIFadd{Hermes }\DIFaddend have not registered significant activity in the correct behaviour.
Without comparing against any other known quantity; the difference between the MTFM trust assessments under ``fair'' and ``malicious'' behaviour is appreciably affected, particularly in looking at the average values returned.


\subsection{\DIFdelbegin \DIFdel{Comparison under dynamic metric weighting}\DIFdelend \DIFaddbegin \DIFadd{Metric Weighting and Significance Analysis}\DIFaddend }
%

We apply a sequence of metric vectors that preferentially weight each metric during \eqref{eq:metric_weighting} to each of the two simulation runs.
...
\caption{$T_{1,MTFM}$ in the All Static case for the \DIFdelbeginFL \DIFdelFL{Selective }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Malicious }\DIFaddendFL Power Control behaviour, emphasising selected metrics and showing the mean and $\pm \sigma$ of $T_{1,MTFM}$ in the same 'fair' scenario}
...
\end{figure}
%
From Fig.~\ref{fig:static_badmouthing} we can see that the selfish node is consistently outside the $\pm\sigma$ envelope of the fair node it's being compared to, particularly TX Power, with smaller impacts on RX/TX Throughput, as would be expected for a power related selfish behaviour. 
...
\DIFaddbegin \todo{Replace with something more relevant}
\DIFaddend 
