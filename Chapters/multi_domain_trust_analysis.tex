...
\end{table}
\todo{FIX: If these tables are worth anything at all, they can be combined into one (i.e. with the targeted behaviour on the left), or alternatively, the top ``10\%'' could me averaged with the ``popularity'' of metrics used rather than a checkmark on each result}
...
\end{figure}
\todo{NOW: Talk about the relationship between the targeted results and the mean optimisation as an excuse to say that it's ok to look across the top performers in most cases}
