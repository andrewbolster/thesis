...
a performance comparison against a selection of current single metric Trust Management Framework (TMF) assessment processes. 
...
\todo{Acoustic Marine Environment Bit}
...
In Section \ref{sec:trustandtmfs} we discuss Trust and TMFs, defining our terminology and reviewing the justifications for the use and development of TMFs for marine acoustic networks\todo{Check consistency of use of this term}.
...
\todo{Why select Hermes and OTMF}
...
\end{align}\todo{Space these out and revert $T_i$ to $x$ and $y$ forms}
...
\subsection{Trust in Marine Networks}\label{sec:trust_in_marine}\todo{Move to Section \ref{sec:trustandtmfs}}
...
\todo{Possibly cite Guo paper on team sizes or find an alternative, but would require tangential discussion of team scaling, which there isn't room for}
...
We also include aggregate assessments; $T_{1,\text{Avg}}$, the unweighted mean of direct trust assessments of $n_1$, $T_{1,\text{Net}}$, that weights assessments according to the network topology from \eqref{eq:networkeffects}, without the whitenisation factor $f_s$, and $T_{1,\text{MTFM}}$, the final MTFM trust assessment value based on both network topology and whitenisation from \eqref{eq:whitenization}.\todo{It's possible this paragraph is surplus to requirements and the relevant sections of the boxplot could be removed to simplify things}
...
\caption{$T_{1,0}$ for Hermes, OTMF and MTFM assessment values for fair and selfish behaviours in the fully mobile scenario (mean of MTFM also shown)}
...
Fig.~\ref{fig:otmf_beta_comparison} shows a comparison between the unweighted response of MTFM compared to OTMF and Hermes assessment functions on the same data for the fair and selfish behaviours respectively.
...
In this case, particularly since the method of attack was not directly related to PLR, OTMF and Hermes have not registered significant activity in the correct behaviour.
...
\subsection{Metric Weighting and Significance Analysis}
%
