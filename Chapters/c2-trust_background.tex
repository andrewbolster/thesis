...
\todo{More trust definitions from the bookmarks list}
...
However, it is arguable that in any network where the operation of that network is not the only concern, or where that network has to interact with any operator, then all of these factors come into play. \todo{Link to autonomy discussion}
...
\todo{Discuss levels of autonomy}
...
\citet{Guo11} demonstrated the ability of \gls{gra} to normalise and combine disparate traits of a communications link such as instantaneous throughput, received signal strength, etc.\ into a \gls{grc}, or a ``trust vector'' in this instance.
