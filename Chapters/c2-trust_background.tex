...
\begin{figure}[!htb]
  ...
  \begin{tikzpicture}[auto, node distance=1.5cm and 0.5cm, line width=2pt, >=latex']
    ...
    \draw [->] (outcomes.south) --++ (0cm,-4cm) -| (percievedFactors.south);

  \end{tikzpicture}
  \caption[Model of Trust]{Model of Trust taken from \cite{Mayer1995}, modelling the }
...
\todo[inline]{Liu and Wang do lots on this\cite{Liu2010} as well as discussion regarding the entropic/probabilistic models of trust. This may be too much to throw in, might inject it later}

\todo[inline]{Talk about trust vs untrust vs nontrust}
...
\todo{Need to discuss how trust is established a) initially among a co-launched group, b) with a newcomer and c) with a returner}
...
\todo[inline]{Expand introduction and plan the rest of the section}
...
This included a particularly interesting development in the form of "Society of Automotive Engineers" (\gls{vsm}) interoperability, whereby existing systems could be grandfathered into STANAG 4586 compliance by the addition of a \gls{vsm} to operate as a protocol translator.
This \gls{vsm} could be mounted on the remote system, utilising a compliant Data Link Interface (DLI), or mounted on the UCS utilising a proprietary DLI to the remote system.
...
\todo[inline]{Standard table}

\todo[inline]{Emphasise Threat Surface discussion}
