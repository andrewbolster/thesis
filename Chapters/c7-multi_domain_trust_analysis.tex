...
The ``best'' weight permutations are shown in \autoref{tab:optmised_weights} for reference.

These weights are applied to untrained simulation data to derive the following results.

An exemplar subset of the results is shows in Figs~\ref{fig:comms_mpc}-~\ref{fig:full_slowcoach}, with the ``misbehaving node'' highlighted with heavier lines, with any observations about the rest of the cohort faded and dashed. For each node assessment, the mean for that assessment over that time period is also included as a solid / dashed line respectively for clarity.

The most intuiativly ``Communications'' behaviour, \gls{mpc}, scores comfortably in the 90th percentile range in both Communications Domain (\autoref{fig:comms_mpc}) and Full Domain (\autoref{fig:full_mpc})  trust assessments. As seen in \autoref{tab:optimised_weights}, both the ``Full'' and ``Comms'' metric optimsations heavily weigh $P_{TX}$, and as this is the metric directly modified by the misbehaviour, it is expected that this is easily discernable using these domain weights.
However when this communications information is unavailable, as is the case in the use of Physical Domain metrics alone in \autoref{fig:phys_mpc}, the misbehaving node (Alfa) is completely undiscernible compared to the other nodes, with all nodes in the cohort tending to a trust value of $0.5$.
How this discernibility would fare under varying emphasis of behaviours is an open question\todo{Answer what happens when you vary MPC power variation}
...
Comparing Figs~\ref{fig:comms_sts} and~\ref{fig:full_sts}, while there is a reasonable dip in the misbehavor's trust assessment, the high level of variance across the cohort is such that this ``mistrust'' triggering is neither consistent or obvious. 
...
\todo{Need to redress potential selection bias in the use for the mean-of-non-outlier-means for $\delta t$ assessment, possibly with an ``Area Under the instantenous mean'' approach; This would also function an as effective ``false positive'' assessment for cross-behaviour weighting}


\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{best_comms_run_MPC}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{best_phys_run_MPC}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{best_full_run_MPC}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{best_phys_run_STS}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{best_phys_run_Shadow}
  \caption{Shadow Physical Metric Trust}
  \label{fig:phys_shadow}
\end{figure}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{best_phys_run_SlowCoach}
  \caption{SlowCoach Physical Metric Trust}
  \label{fig:phys_slowcoach}
\end{figure}
...
\end{table}
\todo{Haven't worked out a clever way of automatically generating both the basic domain and alternate domain texes easily}
