...
\title{Multi-domain Grey Trust with Sparse, Asynchronous, Metric Vectors \\ {\large DRAFT for 8pg submission to TrustCom 15 - 31 March}}
...
Using Grey System Theory, Sequence Buffer operations and we show that by performing per-domain filtering \todo{is there a better phrase for 'filling in the gaps'} while performing cross-domain vector whitenization, a more stable and practical trust assessment is produced. 
...
Black and White numbers are the extremes of this classification; such that $\overset\bullet\oplus \in [-\infty, +\infty]$ and $\overset\circ\oplus \in [x, x] | x \in \mathbb{R}$ or $\oplus(x)$
It is clear that white numbers such as $\overset\circ\oplus$ have a field of zero while black numbers have an infinite field.
...
Therefore, the implemenation of a suitable sequence buffer version of the framework would be beneficial.

Such a sequence buffer framework would involve a tracking predictor that would provide best-guess estimates of an interpolated value for a metric between value updates, and a back-propogation algorithm to retroactively update historical assessments of that metrics so as to better inform any abstracted trust value predictor.
