...
Network saturation is the point at which a network can no longer successfully deliver the offered load presented to it to the relevant destinations (throughput), and is characterised by a peak and a subsequent decline in the throughput of the network when varying the packet emission rate. 
...
Results are also included from the remaining mobility cases (all nodes mobile; all-but-one node mobile; single mobile node), however from Figs.\ref{fig:emission_bella_all_mobile}-~\ref{fig:emission_bella_single_mobile} that the throughput threshold behaviour is qualatitively similar regardless of mobility for this initial node separation.
...
\end{figure}\todo[inline]{I have no idea why A is different to the rest\ldots}
...
\begin{figure}[tp!]
  ...
  \end{subfigure}
  \caption{Network performance varying packet separation for the static case}
...
\begin{figure}[bp!]
  ...
  \end{subfigure}
  \caption{Network performance varying packet separation for the all mobile case}
...
\begin{figure}[tp!]
  ...
  \end{subfigure}
  \caption{Network performance varying packet separation for the all-but-one mobile case}
...
\begin{table}[H]
    ...
    \input{Figures/input/emission_packet_stats_bella_single_mobile}
	\end{center}
\end{table}
