...
\fi
In this chapter we explore the current literature and research around the concepts, theory, and application around Trust and Trust Management, specifically leaning towards the applications of Trust within Autonomous \acrshortpl{manet}.
...
In the third section, current use and applications of Trusted operation of \acrshortpl{manet} is explored, including current \acrfullpl{tmf}.
...
As such it is important to explore the following areas of Trust definition before approaching the application of Trust towards Autonomous Systems and finally to \acrshort{manet}s:
...
While Autonomy is largely taken to be a robotics term based in the case of one human operator and one robotic entity, recent advances in the development of more generalised cyber-physical systems has expanded this definition; from over-the-horizon human operation of \acrshortpl{uav} to global networks of collaborating machines such as Google and beyond.
...
\section{Trust in \acrshortpl{manet}}
...
The distributed and dynamic nature of \acrshortpl{manet} mean that it is difficult to maintain a trusted third party (TTP) or evidence based trust system such as Certificate Authorities or using Public Key Infrastructures (PKI).\todo{possibly worthwhile doing more background on the operation of these}
...
Previous research has established the advantages of implementing TMFs in 802.11 based \acrshortpl{manet}, particularly in terms of preventing selfish operation in collaborative systems \cite{Li2007}, and maintaining throughput in the presence of malicious actors \cite{Buchegger2002}
...
\begin{itemize}
  ...
  \item The trust model should be self-organized.
    \acrshortpl{manet} are characterized to have dynamic, random, rapidly changing and multi-hop topologies composed of variably bandwidth-constrained links
\end{itemize}
