...
\begin{figure}[h]
  \centering
  \subfloat[Static]{\includegraphics[width=\linewidth]{img/throughput_sep_lines_static} \label{fig:throughput_static}}
  \newline
  \subfloat[Mobile]{\includegraphics[width=\linewidth]{img/throughput_sep_lines_all_mobile} \label{fig:throughput_all_mobile}}
...
\todo{Need to reference the bare differences first, this is totally upside down, the MTFM /OTMF could come *before* the weight}
Without comparing against any other known quantity; the difference between the trust assessments between ``fair'' and ``selfish'' behaviour is still appreciably affected.

As per \cite{Guo11}, parallel simulations were performed where there was no malicious behaviour, the ``fair'' scenario. 
...
%
\begin{figure}[h]
  \subfloat[Fair Scenario]{\includegraphics[width=.95\linewidth]{img/trust_beta_otmf_fair} \label{fig:all_mobile_fair_beta}}
  \newline
  \subfloat[Selective Power Control Scenario]{\includegraphics[width=.95\linewidth]{img/trust_beta_otmf_selfish} \label{fig:all_mobile_badmouthing_beta}}
\caption{$T_{1,0}$ for Beta, OTMF and MTFM assessment values for fair and selfish behaviours in the fully mobile scenario}
\label{fig:otmf_beta_comparison}\todo{Add Means and possibly filtering to mtfm}
\end{figure}
%
