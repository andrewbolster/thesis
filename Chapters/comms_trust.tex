...
\todo{Change the purple in these graphs to something less blue}
...
\begin{table}[h]
	...
	\end{center}
\end{table}
\todo{Change ``Delivery'' to ``Propogation'' in table generation}
...
\begin{enumerate}
	\item \acrfull{mpc}, where $n_1$ increases its transmit and forwarding power by 20\% for all nodes \emph{except} communications from $n_0$ in order to make $n_0$ appear to be selfishly conserving energy to the rest of the team, while $n_1$ itself appears to be performing very well.\todo{Justify 20\%}
