...
Upon this per-node assessment methodology, OTMF overlays a observation distribution protocol so as to make the measurements $\alpha_i$ and $\beta_i$ be representative of the direct and 1-hop networks observations of the target node $i$, as well as expiring old observations from assessment and eliminating observations from ``untrustworthy'' nodes.\todo{Not sure if it's suitable for in here but I have serious concerns about the derivations of the 'malicious reporter' test in OTMF: $m$ is not defined anywhere other than being a "deviation threshold" which means nothing. Also OTMF does not weight based on direct or secondary reporting}
...
\begin{figure}[h]
  \centering
  \subfloat[Delay]{\includegraphics[width=.45\linewidth]{img/trust_bella_all_mobile_emph_ADelay_BadMouthingPowerControl} \label{fig:all_mobile_badmouthing_delay}}
  \subfloat[PLR]{\includegraphics[width=.45\linewidth]{img/trust_bella_all_mobile_emph_PLR_BadMouthingPowerControl}\label{fig:all_mobile_badmouthing_plr}}
  \newline
  \subfloat[RX Power]{\includegraphics[width=.45\linewidth]{img/trust_bella_all_mobile_emph_ARXP_BadMouthingPowerControl} \label{fig:all_mobile_badmouthing_rxp}}
...
\begin{figure}[h]
  \centering
  \subfloat[Delay]{\includegraphics[width=.45\linewidth]{img/trust_bella_all_mobile_emph_ADelay_SelfishTargetSelection} \label{fig:all_mobile_selfish_delay}}
  \subfloat[PLR]{\includegraphics[width=.45\linewidth]{img/trust_bella_all_mobile_emph_PLR_SelfishTargetSelection}\label{fig:all_mobile_selfish_plr}}
  \newline
  \subfloat[RX Power]{\includegraphics[width=.45\linewidth]{img/trust_bella_all_mobile_emph_ARXP_SelfishTargetSelection} \label{fig:all_mobile_selfish_rxp}}
  \subfloat[TX Power]{\includegraphics[width=.45\linewidth]{img/trust_bella_all_mobile_emph_ATXP_SelfishTargetSelection}\label{fig:all_mobile_selfish_txp}}
  \newline
  \subfloat[RX Throughput]{\includegraphics[width=.45\linewidth]{img/trust_bella_all_mobile_emph_RXThroughput_SelfishTargetSelection} \label{fig:all_mobile_selfish_rxthroughput}}
  \subfloat[TX Throughput]{\includegraphics[width=.45\linewidth]{img/trust_bella_all_mobile_emph_TXThroughput_SelfishTargetSelection} \label{fig:all_mobile_selfish_txthroughput}}
