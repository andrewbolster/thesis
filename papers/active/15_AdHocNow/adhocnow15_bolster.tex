...
\begin{itemize}
  ...
  \item \emph{CONFIDANCE}; \todo{Add \cite{Buchegger2002} in here if there's space, or find something more recent built on it}
\end{itemize}
...
\end{equation}
\todo{Rewrite and extend to praise/critique Guo}For applications involving low fidelity, temporally sparse metrics with unknown statistical distributions, GRA is a more stable comparative analysis, providing an interval of potential trust values rather than fuzzy-logic or the Bayesian-Beta distributions found in current TMFs \cite{Liu2006}.
...
\begin{table}[H]
  ...
  \begin{center}
      ...
      \input{./input/range_delay_table.tex}
  \end{center}
  \inline{Fix ideal prop time to include packet in the air time}
\end{table}
