...
This novel approach opens the possibility of bringing information about both the physical and communications behaviours of autonomous MANETs together to strengthen and expand the application of future Trust Management Frameworks in sparce and/or resource constrained environments.
...
Early attempts to secure and protect the integrity of Mobile Ad-hoc Networks have relied on some form of strong-cryptographic methods to protect information being sent across it from tampering or malicious inspection.
While such approaches do protect the integrity of individual pieces of data, the increased computation, and storage requirements of modern, strong, decentralised cryptographic systems presents a clear avenue for Denial of Service (DoS) attacks on MANETs2012This thrl()<++>eat is particularly relevant in resource-constrained networks, where one or more aspects of the environment are limited, be it available power, locomotive mobility, data storage, onboard processing, available bandwidth, and channel resources such as capacity and delay.
...
With Trust being reliant on the near-real-time awareness of some behaviour, and cryptography on the pre-establishment of some entropy store and the repeated reinforcement of that numerical security, they represent two very different approaches to system integrity with very different costs/benefits.
...
\begin{equation}
  ...
  \begin{cases}
    % Control
    Q^{95}(\text{INHD,INDD,Speed}) = \emptyset,& \text{Control}\\
    % Shadow
    \begin{aligned}
    Q^{95}(\text{INHD,INDD,Speed}) \neq& \emptyset\\
    \land \text{Speed} \leq& 1.75
    \end{aligned},& \text{Shadow}\\
    % SlowCoach
    \begin{aligned}
    Q^{95}(\text{INHD,INDD,Speed}) &\neq \emptyset\\
    \land \text{Speed} &> 1.75
    \end{aligned},& \text{SlowCoach}\\
  \end{cases}
  \label{eq:classifier}
\end{equation}
